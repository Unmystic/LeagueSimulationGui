...
def compare_results(team , hof_table):
    if team['gamesPlayed'] > hof_table[-1]['gamesPlayed']:
        hof_table.pop(-1)
        hof_table.append({'name': team['name'], 'rating': team['rating'], 'gamesPlayed': team['gamesPlayed'],
                    'goalDifference': team['goalDifference'], 'wins': team['wins'], 'draws': team['draws'],
                    'points': team['points']})
